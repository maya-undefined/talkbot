...
def _is_missing_memory_schema(exc: ProgrammingError) -> bool:
    """Return whether the raised DB error indicates missing memory tables."""
    ...
    return (
        'relation "memory_items" does not exist' in detail
        or 'relation "sessions" does not exist' in detail
    )
...
class MemoryManager:
    ...
    def retrieve(
        ...
        if SessionLocal is None:
            return _rank_memories(self._fallback_candidates(tenant_id, session_id), query_tokens, top_k)
        ...
        try:
            ...
            if not _is_missing_memory_schema(exc):
                raise
            candidates = self._fallback_candidates(tenant_id, session_id)
    ...
    def summarize_session(self, session_id: str, tenant_id: str) -> str:
        ...
        if SessionLocal is None:
            return self._summarize_fallback(session_id, tenant_id)
    ...
    def _fallback_candidates(self, tenant_id: str, session_id: str) -> list[MemoryItem]:
        """Return fallback memory candidates for a tenant/session pair."""

        return [
            item
            for item in self._fallback_items
            if item.tenant_id == tenant_id and (item.session_id == session_id or item.session_id is None)
        ]

    def _summarize_fallback(self, session_id: str, tenant_id: str) -> str:
        """Summarize session memories from fallback storage."""
